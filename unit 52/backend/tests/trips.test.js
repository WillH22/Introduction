...
describe("POST /:username/new_trip", function () {
  test("works", async function () {
    const res = await request(app)
      .post("/trips/user1/new_trip")
      .send({
        trip: {
          name: "newTrip",
          distance: 99,
        },
        locations: [{ location: "test3" }, { location: "test1" }],
      })
...
describe("PATCH /:username/:trip_id/rate", function () {
  test("works", async function () {
    const res = await request(app)
      .patch(`/trips/user1/${testTrips[0].id}/rate`)
      .send({ rating: 1 })
      .set("authorization", `Bearer ${u1Token}`);
    expect(res.statusCode).toEqual(200);
    expect(res.body).toEqual({
      tripId: testTrips[0].id,
      username: "user1",
      tripRating: "1",
    });
  });
...
describe("DELETE /:username/:trip_id/delete", function () {
  test("works", async function () {
    const res = await request(app)
      .delete(`/trips/user1/${testTrips[0].id}/delete`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(res.statusCode).toEqual(200);
  ...
  test("fails not logged in", async function () {
    const res = await request(app).delete(
      `/trips/user1/${testTrips[0].id}/delete`
    );
    expect(res.statusCode).toEqual(401);
  });
});
