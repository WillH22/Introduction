...
describe("POST /new_location", function () {
  test("works", async function () {
    const res = await request(app).post("/locations/new_location").send({
      id: "newloc",
      title: "newtitle",
      latitude: 12,
      longitude: 32,
...
describe("PATCH /rating/:username", function () {
  ...
  test("fails not logged in", async function () {
    const res = await request(app).patch("/locations/rating/user1").send({
      id: "test1",
      username: "user1",
      rating: 1,
    });
...
describe("GET /:location/:username", function () {
  ...
  test("works location not rated", async function () {
    const res = await request(app)
      .get("/locations/test2/user1")
      .set("authorization", `Bearer ${u1Token}`);
    expect(res.statusCode).toEqual(200);
    expect(res.body).toEqual({
      userRating: "You haven't rated this location yet.",
