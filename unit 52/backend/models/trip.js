...
class Trip {
  ...
  static async addTrip(username, { name, distance }) {
    ...
    /** Adds the trip into the user_trips table to store who created the trip */
    await db.query(
  ...
  /** Adds a location to a trip, along with its position in the order it was entered.
   * Returns all entered info, along with the location name.
   */
  ...
  static async getUserTrips(username) {
    ...
    /** If there are no user trips, return an empty array */
    if (!userTrips[0]) {
      return userTrips;
    }

    /** Gets an array of locations for each trip retrieved above */
  ...
  /** Gets the 20 most recent trips entered by anyone into the database */
  static async getRecentTrips() {
    ...
    /** If there are no recent trips, return an empty array */
    if (!trips[0]) {
      return trips;
    }
  ...
  /** Records the user's rating for a trip */
