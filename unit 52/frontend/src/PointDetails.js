...
function PointDetails({ point, username, isMobile }) {
  ...
  return (
    <>
      {point.images.data[0] && (
        <img
          src={point.images.data[0].thumbnail_url}
          alt={point.name}
      ...
      {username && (
        <p>
          <span className="detailTitle">Your Rating:</span> {userRating}
        </p>
      )}
      {username && (
        <p>
          <span className="detailTitle">WeTrip Avg Rating: </span>
          {overallRating}
        </p>
      )}
      {username && userRating && (
      ...
      {point.rating && (
        <p>
          <span className="detailTitle">Marinas.com rating: </span>
          {point.rating} out of {point.review_count} reviews
        </p>
      )}
      <Link
        to={{ pathname: `${point.web_url}` }}
        target="blank"
        className="link"
      >
        View on Marinas.com
      ...
      {showMap && (
        <PointMap
          longLat={[point.location.lon, point.location.lat]}
