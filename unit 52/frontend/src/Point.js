...
function Point({
  ...
  const toggle = async () => {
    // Retrieve detailed information for the point if needed
  ...
  const addPoint = async () => {
    ...
    if (currentTrip[0]) {
      ...
      newDistance =
        MapboxApi.getDistance(previousLocation, newLocation) +
    ...
    localStorage.setItem(
      "currentTrip",
      JSON.stringify([...currentTrip, newPoint])
    );
    ...
    await WeTripApi.addLocation(data);
  };
  ...
  return (
    <div className={isSaved ? "savedPoint" : "searchedPoint"}>
      ...
      {point.kind && (
        <p>
          <span className="detailTitle">Location type: </span>
      ...
      <Collapse isOpen={showDetails}>
        {showDetails && (point.kind || retrievedPoint) && (
          <PointDetails
            point={isSaved ? retrievedPoint : point}
            username={username}
            isMobile={isMobile}
          />
        )}
      ...
      {searchHere && (
        <Button
          onClick={() => searchHere(coordinates, units)}
      ...
      {username && !isSaved && (
        ...
        >
          {currentTrip[0] && currentTrip[currentTrip.length - 1].id === point.id
            ? "Remove From Trip"
            : currentTrip[0]
            ? "Add to Trip"
            : "Launch New Trip"}
